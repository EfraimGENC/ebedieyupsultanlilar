...
const resolveColor = (colorValue: string): string => {
  // If it's already a valid CSS color (hex, rgb, hsl, named color), return as is
  if (colorValue.startsWith('#') || colorValue.startsWith('rgb') || colorValue.startsWith('hsl') ||
    ['red', 'blue', 'green', 'white', 'black', 'yellow', 'purple', 'orange', 'pink', 'gray', 'grey'].includes(colorValue.toLowerCase())) {
    return colorValue;
  }

  // Try to resolve as Tailwind color (e.g., 'warning-700', 'blue-500')
  // Create a temporary element to test the color
  const tempElement = document.createElement('div');
  tempElement.style.display = 'none';
  tempElement.className = `text-${colorValue}`;
  ...
  try {
    ...
    // If the color was resolved successfully (not the default), use it
    if (resolvedColor && resolvedColor !== 'rgba(0, 0, 0, 0)' && resolvedColor !== 'rgb(0, 0, 0)') {
      return resolvedColor;
    }

    // Try with background color class as fallback
    tempElement.className = `bg-${colorValue}`;
    const bgColor = getComputedStyle(tempElement).backgroundColor;
    if (bgColor && bgColor !== 'rgba(0, 0, 0, 0)' && bgColor !== 'rgb(0, 0, 0)') {
      return bgColor;
    }

    // Try CSS custom property (e.g., --warning-700)
    const customProperty = `--${colorValue}`;
    const rootStyles = getComputedStyle(document.documentElement);
    const customValue = rootStyles.getPropertyValue(customProperty);
    if (customValue) {
  ...
  console.warn(`Could not resolve color: ${colorValue}, falling back to original value`);
  return colorValue;
};
